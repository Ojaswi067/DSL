...
Class : SE-II(R-Batch)
'''
#Problem Statement A5

"""
Write a Python program to compute following operation on String:
a) To display word with the longest length.
b) To determines the frequency of occurrence of particular character in the string.
c) To check whether given string is palindrome or not.
d) To display index of the first appearance of the substring. 
e) To count the occurrence of each word in a given string.
"""

def Display_word_longest_lenght():
  ...
  while( i < len(Str)):
    ...
    if(len(M_str) < len(word)):
         M_str = word 
  print("\tWord with longest lenght is the %s having lenght %d\n\n"%(M_str,len(M_str)))


def Determine_frequency_of_particular_character_in_string() :
...
def main():
 while True:
    ...
    print("\t\t2 : Determines the frequency of occurrence of particular character in the string.")
    print("\t\t3 : Check whether given string is palindrome or not")
    print("\t\t4 : Display index of the first appearance of the substring.")
    ...
    if (ch==6):
       ...
       quit()
        ...
        Display_word_longest_lenght()
    elif(ch == 2):
        Determine_frequency_of_particular_character_in_string()
    elif(ch == 3):
